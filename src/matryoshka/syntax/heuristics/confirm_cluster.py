...
class BuildCluster(Heuristic):
    ...
    def generate_description(self, entries, few_shot_set=None, **kwargs):
        ...
        if self.use_description_distance:
            ...
            print("Calling Gemini on {} tasks".format(len(tasks)))
            ...
            for candidate in candidates:
                try:
                    description = re.sub(
                        r"\s+",
                        " ",
                        json.loads(candidate.candidates[0].strip()),
                    )
    ...
    def query(self, entries, prefix=None, **kwargs):
        ...
        if new_tasks:
            ...
            for idx in to_be_fixed:
                response_content[idx] = corrected_candidates.pop(0).candidates[
                    0
                ]
