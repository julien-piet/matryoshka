...
class Template:
    ...
    def convert_to_wildcard_template(self, name=False, var_mapping=None):
        """Return a string in which each variable is replaced with a wildcard"""
        if name and not var_mapping:
            raise ValueError(
    ...
    def load_from_json(tags, entry):
        ...
        elements = [
            e for e in elements if e.value.strip() or e.trailing_whitespace
        ]
    ...
    def load_array_from_response(
        ...
        # Remove empty elements
        ret_elements = [
            e for e in ret_elements if e.value.strip() or e.trailing_whitespace
        ]
