...
from ..classes import Module, OCSFMapping, Parser, VariableSemantics
...
from ..utils.prompts.mapping.map import gen_prompt as gen_confirm_prompt
...
class MapToAttributes(Module):
    ...
    def __init__(
        ...
        self.client = (
            OCSFSchemaClient(caller, os.path.join(cache_dir, "OCSF"))
    ...
    def _save_dill(self):
        with open(
            os.path.join(self.output_dir, "parsed.dill"),
            "wb",
        ) as f:
        ...
        **kwargs,
        ...
        if text_description:
            targets = [
                ...
                if self.client.generated_descriptions[k].embedding is not None
            ]
    ...
    def _get_sibling_nodes(self, element_ids, event, source_embedding):
        ...
        try:
            sim = torch.nn.CosineSimilarity()(
                source_embedding, target_embeddings
            )
        except:
            return
        closest = sim.topk(
    ...
    def matching(
        ...
        **kwargs,
        ...
        try:
            ...
            get_logger().error(
                "Error getting prompt parameters for elements %s and event %s",
                element_ids,
                event,
            )
            return False
