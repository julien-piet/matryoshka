...
from matryoshka.utils.prompts.mapping.OCSF.map import (
    gen_prompt as gen_confirm_prompt,
)
from matryoshka.validation.ocsf import MappingValidator
...
class MapToAttributes(Module):
    ...
    def get_event_types(self, element_ids):
        if not isinstance(element_ids, list):
            element_ids = [element_ids]

        event_types = set()
        for element_id in element_ids:
            template_ids = self.tree.templates_per_node[element_id]
    ...
    def filtering(
        ...
        **kwargs,
        ...
        events = self.get_event_types(element_ids)
        target_type = (
        ...
        if text_description:
            targets = [
                ...
                if self.client.attributes[k].embedding is not None
            ]
            dest_embeddings = torch.tensor([t[1] for t in targets])

            try:
                sim = torch.nn.CosineSimilarity()(
                    source_embedding, dest_embeddings
                )
            except:
                breakpoint()
    ...
    def _get_sibling_nodes(self, element_ids, source_embedding, target_type):
        # Get sibling types
        sibling_nodes = {
            i.id
            for element_id in element_ids
        ...
        sibling_mapping = self.client.get_siblings(sibling_fields, target_type)
    ...
    def matching(
        ...
        **kwargs,
        ...
        selected_fields = [p for p in field_counts if field_counts[p]]
    ...
    def process(self, log_file=None, only_validate=False, **kwargs):
        ...
        for cluster in tqdm(
            ...
            self.filtering(element_ids, fuzzy=1)
