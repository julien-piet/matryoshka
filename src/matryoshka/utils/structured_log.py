...
class TreeEditor:
    ...
    def tree_stats(self):
        ...
        created_field_names = {
            ...
            for n_id in variable_nodes
            if n_id in self.var_mapping
            and self.var_mapping[n_id].created_attribute
        }
        ocsf_mappings = set()
        for var in variable_nodes:
            if not self.var_mapping[var] or not self.var_mapping[var].mappings:
                continue
            for mapping in self.var_mapping[var].mappings.values():
    ...
    def line_to_dict(self, line_id, OCSF=False, trunc_OCSF=True):
        rtn_dict = {}
        ...
        for node_id in nodes:
            if node_id not in self.var_mapping:
                continue
            created_field_name = self.var_mapping[node_id].created_attribute
            if not OCSF and created_field_name:
                field_names[created_field_name].add(node_id)
            else:
                # Add all created fields as unmapped
                if created_field_name:
                    field_names[f"unmapped.{created_field_name}"].add(node_id)

                # Add all OCSF mappings
        ...
        for field_name, node_ids in field_names.items():
            ...
            for node_id in node_ids:
                values.append(self.node_id_to_match_per_line[node_id][line_id])
            if len(values) > 1:
                rtn_dict[field_name] = values
            elif values:
                rtn_dict[field_name] = values[0]
    ...
    def export(self, OCSF=False):
        ...
        for line_id, line in enumerate(self.lines):
            ...
            for node_id in matched_nodes:
                ...
                if (
                    node_id in self.var_mapping
                    and self.var_mapping[node_id].mappings
                ):
                    for event in OCSF_events:
                        if event in self.var_mapping[node_id].mappings:
                            for field in (
                                self.var_mapping[node_id]
                                .mappings[event]
                                .field_list
                            ):
    ...
    def match_attribute(
        ...
        # Add OCSF attributes
        ocsf_attributes = [
            attr.split(".", 1)[1] for attr in attribute_name if "." in attr
        ]
        all_matching_attributes = [
            attr
            for attr in self.attribute_to_node_id
            if "." in attr
            and any(
                attr.split(".", 1)[1] == ocsf_attr
                for ocsf_attr in ocsf_attributes
            )
        ]
        attribute_name = [
            attr for attr in attribute_name if attr not in ocsf_attributes
        ] + all_matching_attributes
        ...
        def populate_field_to_ocsf(target_dict, target_editor):
            for _, var in target_editor.var_mapping.items():
                ...
                if var.created_attribute not in target_dict:
                    # If there are no mappings, we still need to add the attribute to avoid missing it
    ...
    def end_to_end_score(self, target_editor, ocsf=False):
        if not isinstance(target_editor, TreeEditor):
            raise ValueError("target_editor must be a TreeEditor object")
