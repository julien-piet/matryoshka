...
Please provide a schema for these log lines.
"""
...
def gen_prompt(elt, field_mapping=None, descriptions=None):
    lines, template = elt
    if not isinstance(template, str):
        template = template.format_as_example(
            force_match_with_entry=True, entry=lines[0], regex=True, ids=True
        )
    lines = "\n".join(lines)
