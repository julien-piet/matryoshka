...
system_old = """You are an expert log parser. You are given a few log lines that have been generated by the same program. These lines have been separated into constant tokens and entities, and each entity has been given a description. Your task is to provide a schema for these log lines that assigns each entity a field name and description. 
* If the original element included a field_name, please reuse it.
* You can group elements into a single field if they belong to a list of elements that play the same role
* If you do decide to group elements with a single field name, you must have a single json entry for the group, with the name you chose, and that lists all the matching ids.
* Use snake case, and use singular nouns for field names.
* Name fields according to their role in the event and their context, not according to their specific content."""
...
consistency_addon = """ Most importantly, make sure to be consistent with previous schemas: any field that has the same purpose or role as in a prior schema should use the same field name. """
