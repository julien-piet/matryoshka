...
from .api import OpenAITask
from .logging import get_logger
from .prompts.OCSF_descriptions import gen_prompt as gen_desc_prompt
from .prompts.OCSF_events import gen_prompt as gen_event_prompt
from .prompts.OCSF_types import gen_prompt as gen_type_prompt
...
class OCSFFieldDescriptor:
    ...
    def generate_description(
        fd, caller, tqdm=False, model="gemini-1.5-flash", **kwargs
    ):
        if not isinstance(fd, list):
            fd = [fd]

        fd = [f for f in fd if f.generated_description == ""]
        ...
        for r_id, resp_array in enumerate(responses):
            resp = resp_array.candidates[0]["content"]
            try:
...
class OCSFEventDescriptor:
    ...
    @staticmethod
    def generate_embeddings(fd, caller):
        if not isinstance(fd, list):
            fd = [fd]

        targets = [f.generated_description.strip() for f in fd]

        tasks = [
            OpenAITask(
                message=t[1].generated_description,
        ...
        for embedding_id, embedding in enumerate(embeddings):
            fd[embedding_id].embedding = embedding
    ...
    def generate_description(
        ...
        tasks = []
        for f in fd:
            user, system = gen_event_prompt(f)
            task = OpenAITask(
                system_prompt=system,
                max_tokens=256,
                model=model,
    ...
    def gen(self, caller):
        self.generate_description(self, caller)
        self.generate_embeddings(self, caller)
...
class OCSFSchemaClient:
    ...
    def get_object(self, object_name, seen_types=None):
        ...
        for attr_name, attr in response.items():
            ...
            attr.attributes = self.get_object(attr.type, seen_types.copy())
    ...
    def build_descriptions(self, caller):
        ...
        OCSFFieldDescriptor.generate_description(
            list(paths.values()), caller, tqdm=True
        )

        # Save the descriptions
        self._save_to_cache()

    def build_event_descriptions(self, caller):
        """
        Builds the descriptions for all events.
        """
        get_logger().info("Building event descriptions...")
        # Get the list of all paths
        events = self.get_classes()

        event_lineage = [self.get_event_lineage(event) for event in events]
        self.event_descriptions = {}
        for event, lineage in zip(events, event_lineage):
            self.event_descriptions[event] = OCSFEventDescriptor(event, lineage)

        # Generate the descriptions
        OCSFEventDescriptor.generate_description(
            event_lineage, caller, tqdm=True
        )
        OCSFE
    ...
    def get_description(self, events, target_class, fuzzy=0):
        """
    ...
    def build_inferred_types(
        self, caller, tqdm=True, model="gemini-1.5-pro", **kwargs
    ):
        """
    ...
    def build_attribute_embeddings(self, caller):
        ...
        for embedding_id, embedding in enumerate(embeddings):
            self.generated_descriptions[targets[embedding_id][0]].embedding = (
                embedding
            )
    ...
    def get_event_lineage(self, event):
        ...
        event_details = all_classes[event]
        if event_details["category"] == "other":
            return [(event, event_details["description"])]
        ...
        if "description" in category_details:
...
class OCSFMapping:
    ...
    def __str__(self):
        return f"""###########################
Field Description: {self.field_description.strip()}
Field List: {json.dumps(self.field_list)}
###########################"""
