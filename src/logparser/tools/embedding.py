...
class NeuralEmbedding(Embedding):
    """
    Neural embedding model
    """

    def __init__(
        self, filt=False, model: str = "sentence-transformers/all-mpnet-base-v2"
    ) -> None:
        super().__init__(filt)
    ...
    def _embed(self, sentences: Union[list[str], str]):
        ...
        rslt = self.embedding.encode(sentences)
    ...
    def __setstate__(self, state):
        ...
        self.embedding = SentenceTransformer(self.model)
        if torch.cuda.is_available():
            self.embedding = self.embedding.to("cuda")
...
class NeuralDistance(Distance):
    ...
    def __init__(
        ...
        super().__init__()
        self.embedding = (
            NeuralEmbedding(filt, model) if not embedding else embedding
        )

    def _embed(self, sentences: Union[list[str], str]):
        return self.embedding(sentences)
...
class NaiveDistance(Distance):
    ...
    def get_template_old(self, entry, k=-1, templates=None, debug=False):
        score = {}

        if templates:
            selected_templates = {
                k: v for k, v in self.templates.items() if k in templates
            }
        else:
            selected_templates = self.templates

        template_embeddings = {}
        for t_idx, t in selected_templates.items():
            full, part = {}, {}
            for elt_id in t:
                elt_full, elt_part = self.elements[elt_id].get()
                for key, value in elt_full.items():
                    if key in full:
                        full[key] += value
                    else:
                ...
                for key, value in elt_part.items():
                    if key in part:
                        ...
                        part[key] = value
            template_embeddings[t_idx] = (full, part)

        for key, (full, partial) in template_embeddings.items():
            ...
            def get_similarity(entry, words):
            ...
            full_ct = get_similarity(entry, full)
            part_ct = get_similarity(entry, partial)
