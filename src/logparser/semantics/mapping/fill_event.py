...
from ...tools.prompts.map_templates import gen_prompt as gen_mapping_prompt
from ...tools.prompts.template_description import (
    ...
    def generate_description(self, template_id, **kwargs):
        ...
        try:
            description = re.sub(
                "\s+",
                " ",
                json.loads(candidates.candidates[0]["content"].strip()),
            )
        except json.JSONDecodeError:
            description = re.sub(
                '(^")|("$)', "", candidates.candidates[0]["content"].strip()
            )
        get_logger().info(
