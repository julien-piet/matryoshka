...
class MapVariables(Module):
    ...
    def generate_description(self, element_id, **kwargs):
    ...
    def filtering(
        ...
        **kwargs,
        ...
        self._get_sibling_nodes(
            element_ids, event, source_embedding=source_embedding
        )
    ...
    def _get_sibling_nodes(self, element_ids, event, source_embedding):
        ...
        for sibling in sibling_mapping:
            if sibling not in {
                r[0]
                for element_id in element_ids
                for r in self.var_mapping[element_id].mappings[event].candidates
            }:
    ...
    def process(self, **kwargs):
        ...
        for element_id in tqdm(
            ...
            if element_id not in self.var_mapping:
                self.generate_description(element_id)
                requires_save = True
        ...
        self.description_distance = DescriptionDistance(
            self.clusters, {k: v.embedding for k, v in self.var_mapping.items()}
        )
