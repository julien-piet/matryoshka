...
def main():
    ...
    if args.config_file:
        ...
        if not args.parser:
            if not args.golden:
                args.parser = config["results_path"] + "_events/parser.dill"
            else:
                args.parser = (
                    config["results_path"] + "_events_fixed/parser.dill"
                )
        if not args.output:
            if not args.golden:
                args.output = config["results_path"] + "mapping"
            else:
                args.output = config["results_path"] + "_mapping_fixed"
