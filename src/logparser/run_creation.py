...
from .semantics.mapping.create_attributes import CreateAttributes
...
def main():
    ...
    if args.config_file:
        ...
        if not args.parser:
            if not args.golden:
                args.parser = config["results_path"] + "_parser/parser.dill"
            else:
                args.parser = (
                    config["results_path"] + "_parser_fixed/parser.dill"
                )
        if not args.output:
            if not args.golden:
